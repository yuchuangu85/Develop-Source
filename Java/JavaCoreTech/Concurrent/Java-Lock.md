...
![img](media/c8703cd9.png)

根据从上面的概念描述我们可以发现：

- 悲观锁适合**写**操作多的场景，先加锁可以保证写操作时数据正确。
- 乐观锁适合**读**操作多的场景，不加锁的特点能够使其读操作的性能大幅提升。
...
![img](media/58fc5bc9.png)

但如果是非可重入锁的话，此时管理员只允许锁和同一个人的一个水桶绑定。第一个水桶和锁绑定打完水之后并不会释放锁，导致第二个水桶不能和锁绑定也无法打水。当前线程出现死锁，整个等待队列中的所有线程都无法被唤醒。

![img](media/ea597a0c.png)
...
>读写锁的使用场景是，读多写少的情况下，写锁是独占锁，独占保证了只能一个线程写，并且其他线程也读等待，目的是保证没有脏读的情况；读锁是共享锁，需要注意的是读锁之间共享，只要当有读锁为释放，写锁也会获取失败，这个目的同样是保证没有脏读的发生。
...
[不可不说的Java“锁”事](https://tech.meituan.com/2018/11/15/java-lock.html)--美团

## 其他：

* [一文看懂Java锁机制](https://juejin.im/post/5e0c5eba6fb9a047ef326a0b)
...
* [最全Java锁详解：独享锁/共享锁+公平锁/非公平锁+乐观锁/悲观锁](https://youzhixueyuan.com/detailed-explanation-of-java-lock.html)
* [锁降级](https://zhuanlan.zhihu.com/p/138274427)
