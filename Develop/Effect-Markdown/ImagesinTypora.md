<h1 align="center">Images in Typora</h1>
...
![drag-img](media/drag-img-20200602142953943.gif)
...
Typora supports paste image data from clipboard, **after telling typora where to put those images**. Typora would put image data into given folder or server, then insert images referring to that stored file or URL. Please refer to section **[When insert local image…](http://support.typora.io/Images/#when-insert-local-image…)** for more detail.
...
Typora supports using [iPic](https://itunes.apple.com/app/id1101244278?ls=1&mt=12) to copy image files into a given folder or web server when inserting local images using drag and drop, or from the menu. The instructions for how to set it up follows.
...
2. Enable `Editor` → `Image Insert` → `Allow copy images to given folder` in preferences panel.
   ...
   ![Snip20161117_2](media/Snip20161117_2.png)

3. Select `Format` → `Image` → `When Insert Local Images` → `Copy Image File to Folder` from the menu bar, and pick the target folder.

   ![Snip20161117_6](media/Snip20161117_6.png)

In step 3, a new item `typora-copy-images-to: {relative path}` will be inserted into the [YAML Front Matter](http://yaml.org/) block of the current document. So you can also manually add **typora-copy-images-to** property in YAML Front Matter to enable this behavior.
...
Please refer [Upload Automatically When Insert Images](http://support.typora.io/Upload-Image/#upload-automatically-when-insert-images).
...
![img](media/upload.gif)

Please refer [Upload Image](http://support.typora.io/Upload-Image).
...
Please refer [Upload All Local Images](http://support.typora.io/Upload-Image/#upload-all-local-images)

### Upload when inserting images

How-tos for this part can be found in section [When insert local image…](http://support.typora.io/Images/#when-insert-local-image…) → Upload image file to web server.
